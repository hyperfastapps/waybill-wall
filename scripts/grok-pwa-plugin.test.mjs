...
/** Tests that omit `site` must not inherit this app's site.json title. */
...
test("names the manifest for non-grok hosts and ships install icons", () => {
  const manifest = JSON.parse(renderWebManifest("waybill-wall.vercel.app"));
  assert.equal(manifest.name, "Waybill Wall");
  assert.equal(manifest.short_name, "Waybill Wall");
  ...
  assert.equal(JSON.parse(renderWebManifest("localhost:8080", {})).name, "Waybill Wall");
});
